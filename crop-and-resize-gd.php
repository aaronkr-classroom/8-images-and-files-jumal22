...
function create_filename($filename, $upload_path)              // Function to make filename
{
    ...
    $basename   = preg_replace('/[^A-Za-z0-9]/', '-', $basename); // Clean basename
    $i          = 0;                                           // Counter
...
function crop_and_resize_image_gd(
    $orig_path, $new_path, $new_width, $new_height
        ...
        if ($new_ratio < $orig_ratio) {
            ...
            $x_offset = ($orig_width - $select_width) / 2; //중간위치 계산
            $y_offset = 0; //(top)
            ...
            $x_offset = 0; // (left)
            $y_offset = ($orig_height - $select_height) / 2; //중간위치 계산 
        }
